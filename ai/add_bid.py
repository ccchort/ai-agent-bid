...
async def generate(data: dict) -> list[dict]:
    ...
    context = [
                {
                    ...
                    'text': (
                        ...
                        "- 'Обращение': точная передача обращения без обобщения, не перефразируй, не придумывай, просто передай сообщение в исходном виде, регестрируй обращение, как есть. Есть исключения для добавления обращения: если массив состоит только из исключений и нет указания документа, то возвращай пустой список []."
                        "Исключения: ок; спасибо; благодарю; хорошо, спасибо; готово; да; нет; отправила; отправлен; смайлики"
                        "- 'Документы': ссылка на Google Документы, если она есть в тексте. Если в массиве сообщений есть: 'Клиент прислал фото', 'Клиент прислал файл', 'Клиент добавил вложение(фото или файл) укажи это в документах, а из обращения вырежи эту фразу. В остальных случаях пустая строка' \"\".\n"
    ...
    if not result.text or result.text.split()[0] == "[]":
        return []
...
if __name__ == "__main__":
    result = asyncio.run(generate({'id': 16, 'user_id': 1414952718, 'platform': 'Max', 'accumulated_text': '😭 Клинт прислал фото', 'created_at': '2026-07-21 22:33:05 +0400', 'last_message_at': '2026-07-21 22:44:33 +0400', 'client_name': 'Бухгалтерия ИП Ватрушкин'}))
    print(result)
