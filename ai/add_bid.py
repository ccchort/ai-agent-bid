    ...
    context = [
        {
            ...
            "text": (
                ...
                "Из текста определи приоритет выполнения задачи и верни 'Нет' или 'Да'. "
                "Если прислали документы(они обычно в виде ссылки на Google Документы), то прикрпи их, если нет, то оставь поле пустым"
                ...
                "'Обращение': 'string',"
                "'Документы': 'string',"
                "'Приоритет выполнения задачи для исполнителя': 'string'}"
                "Если сами сообщения не несут в себе никакого смысла, возвращай пустой список []"
            )
        },
        {
            'role': 'user',
...
if __name__ == "__main__":
    result = asyncio.run(generate({'id': 16, 'user_id': 1414952718, 'platform': 'Telegram', 'accumulated_text': 'оплатить счет', 'created_at': '2026-07-21 22:33:05 +0400', 'last_message_at': '2026-07-21 22:44:33 +0400', 'client_name': 'ИП Малуева. Бухгалтерия'}))
    print(result)
